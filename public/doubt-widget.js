(function () {
  ...
  widget.innerHTML = [
    ...
    '<div id="doubtPanel">',
      '<div class="dw-header">',
        '<div class="dw-header-left">',
          '<div class="dw-avatar">AI</div>',
          '<div>',
            '<div class="dw-title">AI Doubt Solver</div>',
            '<div class="dw-sub"><span class="dw-online"></span> Available 24/7</div>',
          '</div>',
        '</div>',
        '<button class="dw-close" onclick="DoubtWidget.toggle()">x</button>',
      '</div>',
      '<div class="dw-ctx-bar" id="dwCtxBar">',
        '<span>Course:</span><span id="dwCtxLabel"></span>',
      '</div>',
    ...
    '<style>',
      ...
      '#doubtToggleBtn{width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#6366f1,#8b5cf6);border:none;cursor:pointer;font-size:16px;color:#fff;box-shadow:0 4px 20px rgba(99,102,241,.4);position:relative;transition:.2s}',
      '#doubtToggleBtn:hover{transform:scale(1.05)}',
      ...
      '.dw-avatar{width:36px;height:36px;background:rgba(255,255,255,.2);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700}',
      ...
      '.dw-chip{background:#f0f0ff;border:1px solid #c7d2fe;color:#4f46e5;padding:4px 10px;border-radius:20px;font-size:11px;cursor:pointer;transition:.15s}',
      ...
      '@media (max-width: 640px){#doubtWidget{right:12px;bottom:12px}#doubtPanel{width:min(92vw,360px);right:0}}',
    '</style>'
  ].join('');
  ...
  input.addEventListener('keydown', function (e) {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      window.DoubtWidget.send();
    }
  });

  function getDefaultChips() {
    if (courseCtx) {
      return ['Explain this topic', 'Give me a code example', 'How do I start?', 'What should I learn next?'];
    }
  ...
  function showWelcome() {
    ...
    appendBot(greeting + " I'm your AI tutor. Ask me about concepts, code, or your recommended courses.");
  ...
  function showChips(chips) {
    ...
    el.innerHTML = chips.map(function (chip) {
      return '<button class="dw-chip" onclick="DoubtWidget.sendText(' + JSON.stringify(chip) + ')">' + chip + '</button>';
    }).join('');
  }
  ...
  window.DoubtWidget = {
    ...
    setCourse: function (course) {
      ...
      var field = document.getElementById('dwInput');

      if (course) {
        label.textContent = course.title;
        bar.classList.add('visible');
        field.placeholder = 'Ask about ' + course.title + '...';
      } else {
        bar.classList.remove('visible');
        field.placeholder = 'Ask anything about your course...';
      }
      ...
      fetch('/api/ask-doubt', {
        ...
        .then(function (data) {
          ...
          if (data.success) {
            ...
            showChips(['Ask another question', 'Give me an example', 'Explain more simply', 'What is next?']);
            return;
          }
        ...
        .catch(function () {
          ...
          isTyping = false;
          document.getElementById('dwSendBtn').disabled = false;
          appendBot('Cannot reach the server right now. Make sure node server.js is running.');
        });
    }
  };
})();
